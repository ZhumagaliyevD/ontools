...
class FFChatInfo {
  ...
  final List<UserRecord>? groupMembers;

  UserRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, UserRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<UserRecord> get otherUsersList => otherUsers.values.toList();
  ...
  Stream<FFChatInfo> getChatInfo({
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => UserRecord.getDocument(ref).first),
      );
